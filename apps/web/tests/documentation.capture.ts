...
import { realAccountPath, workspaceRoot } from "./orchestrator";
...
test("captures the documentation media from the running application", async ({
  ...
  const assertNoRealAccountPath = async (label: string): Promise<void> => {
    const rendered = await page.locator("body").innerText();
    const offending = realAccountPath(rendered);
    expect(
      offending,
      `${label} renders the real account path ${offending ?? ""}. Re-run the capture with HEIKAS_DEMO_ROOT set to a neutral directory.`,
    ).toBeNull();
  };
  ...
  await assertNoRealAccountPath("the dashboard screen");
  ...
  await page.waitForTimeout(600);
  await assertNoRealAccountPath("the plan approval screen");
  ...
  await assertNoRealAccountPath("the live timeline");
  await hold(8, 260);
  ...
  await assertNoRealAccountPath("the run detail screen");
  ...
  await assertNoRealAccountPath("the candidate comparison screen");
